...
function useTocState(items: TocItem[]) {
  ...
  const toggleH2 = (id: string) => {
    setExpandedH2s(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; });
  };

  return { active, expandedH2s, groups, scrollTo, toggleH2 };
}

function TocList({ items }: { items: TocItem[] }) {
  const { active, expandedH2s, groups, scrollTo, toggleH2 } = useTocState(items);
  if (items.length === 0) return null;
  return (
    <ul className="px-4 py-3 space-y-0.5 max-h-[65vh] overflow-y-auto">
      {groups.map(({ h2, h3s }) => {
          ...
          <li key={h2.id}>
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className={`flex-1 text-left py-1 text-[13px] leading-snug transition-colors ${isH2Active || hasH3Active ? "text-blue-600 font-semibold" : "text-slate-700 hover:text-blue-600"}`}
              >
                {h2.text}
              </button>
            </div>
...
function TableOfContents({ items }: { items: TocItem[] }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 overflow-hidden mb-6 bg-white shadow-sm">
      ...
      >
        <div className="flex items-center gap-2.5 text-slate-800 font-bold text-[13px] uppercase tracking-wider">
      ...
      {open && <TocList items={items} />}
    </div>
  );
}

/* ── Floating TOC button (mobile + đọc bài trên desktop) ── */
function FloatingToc({ items }: { items: TocItem[] }) {
  ...
  return (
    <div
      ref={panelRef}
      className={`fixed bottom-5 right-5 z-40 transition-all duration-300 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}
    >
      {/* Expanded panel */}
      {open && (
        <div className="absolute bottom-14 right-0 w-72 sm:w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-in slide-in-from-bottom-2 fade-in duration-200">
      ...
      <button
        onClick={() => setOpen(!open)}
        className={`w-12 h-12 rounded-full shadow-lg flex items-center justify-center transition-all duration-200 ${open ? "bg-violet-600 text-white rotate-90" : "bg-white text-blue-600 hover:bg-blue-600 hover:text-white border border-slate-200"}`}
      ...
      <div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          <div className="grid lg:grid-cols-[1fr_300px] gap-6 lg:gap-10">
            ...
            {/* ── Sidebar Desktop ── */}
            {/* self-start + sticky: sidebar không bị kéo dài theo main column */}
            <aside className="hidden lg:block self-start sticky top-24 max-h-[calc(100vh-6.5rem)] overflow-y-auto space-y-5 pr-1">
              <div>
