...
export default function Footer() {
  ...
  const youtube  = s.contact_youtube   || "https://www.youtube.com/@hoccungson116";
  const tiktok   = s.contact_tiktok   || "";   // set trong Supabase site_settings: contact_tiktok
  const linkedin = s.contact_linkedin  || "";   // set trong Supabase site_settings: contact_linkedin
  ...
  // Chỉ hiển thị social icon nếu có URL hợp lệ (không dùng href="#")
  const allSocials = [
    {
      label: "Facebook",
    ...
    // TikTok — chỉ hiện nếu contact_tiktok đã được set trong Supabase site_settings
    ...(tiktok ? [{
      label: "TikTok",
      href: tiktok.startsWith("http") ? tiktok : `https://www.tiktok.com/@${tiktok}`,
      icon: (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
    ...
    // LinkedIn — chỉ hiện nếu contact_linkedin đã được set trong Supabase site_settings
    ...(linkedin ? [{
      label: "LinkedIn",
